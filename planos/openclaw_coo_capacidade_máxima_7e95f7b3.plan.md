...
- **CMO:** LinkedIn, Instagram, conteúdo
...
1. Executar `openclaw memory index` para reindexar
2. Validar com `openclaw memory search "organograma COO responsabilidades"`
...
## Reversibilidade (em caso de erro)

**Antes de executar:** Criar backup com `cp` ou `git commit` (se workspace for repo git).


| Fase                   | Backup                                                                 | Como reverter                                                                                                                                             |
| ---------------------- | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1. BOOTSTRAP.md**    | `cp BOOTSTRAP.md BOOTSTRAP.md.bak`                                     | `cp BOOTSTRAP.md.bak BOOTSTRAP.md`                                                                                                                        |
| **2. extraPaths**      | `cp ~/.openclaw/openclaw.json ~/.openclaw/openclaw.json.bak`           | Remover o bloco `memorySearch.extraPaths` do openclaw.json. O main.sqlite mantém os chunks antigos; a busca simplesmente não incluirá os paths removidos. |
| **2. main.sqlite**     | `cp ~/.openclaw/memory/main.sqlite ~/.openclaw/memory/main.sqlite.bak` | `cp main.sqlite.bak main.sqlite` (restaura índice completo ao estado anterior)                                                                            |
| **3. n8n-trigger**     | —                                                                      | `rm tools/n8n-trigger.sh` e reverter alterações em TOOLS.md                                                                                               |
| **4. AGENTS.md**       | `cp AGENTS.md AGENTS.md.bak`                                           | `cp AGENTS.md.bak AGENTS.md`                                                                                                                              |
| **5. platform-status** | `cp memory/platform-status.json memory/platform-status.json.bak`       | `cp platform-status.json.bak platform-status.json`                                                                                                        |


**Comando único de backup (executar antes de começar):**

```bash
cd /root/.openclaw/workspace
cp BOOTSTRAP.md BOOTSTRAP.md.bak
cp AGENTS.md AGENTS.md.bak
cp memory/platform-status.json memory/platform-status.json.bak
cp ~/.openclaw/openclaw.json ~/.openclaw/openclaw.json.bak
cp ~/.openclaw/memory/main.sqlite ~/.openclaw/memory/main.sqlite.bak
```

**Reverter tudo:** Restaurar os 5 arquivos do backup. Deletar `tools/n8n-trigger.sh` e `docs-repo-https/tech/n8n-webhooks.md` se criados.
